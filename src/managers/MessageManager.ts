...
type RawMessageData = { id: Snowflake } & Record<string, unknown>;
...
// Type for the channel parameter
interface TextBasedChannel {
  id: Snowflake;
  client: import('../client/Client').default;
  guildId?: Snowflake | null;
  guild?: { channels: { cache: { get: (id: Snowflake) => unknown } } };
  [key: string]: unknown;
}

class MessageManager extends CachedManager<Snowflake, Message, MessageResolvable, RawMessageData> {
   ...
   *   .catch(console.error);
   */
  async fetchPinned(cache: boolean = true): Promise<Collection<Snowflake, Message>> {
   ...
   */
  async edit(message: MessageResolvable, options: unknown): Promise<Message> {
   ...
   * @returns {Promise<void>}
   */
  async react(message: MessageResolvable, emoji: unknown, burst: boolean = false): Promise<void> {
    message = this.resolveId(message);
    if (!message) throw new TypeError('INVALID_TYPE', 'message', 'MessageResolvable');
