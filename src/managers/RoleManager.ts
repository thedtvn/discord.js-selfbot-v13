...
type RawRoleData = { id: Snowflake } & Record<string, unknown>;
 ...
 */
class RoleManager extends CachedManager<Snowflake, Role, RoleResolvable, RawRoleData, [Guild]> {
   ...
   *   .catch(console.error);
   */
  async fetch(id?: Snowflake, { cache = true, force = false } = {}): Promise<Role | null | Collection<Snowflake, Role>> {
    if (id && !force) {
   ...
   */
  async fetchMemberCounts(): Promise<Record<Snowflake, number>> {
   ...
   */
  async fetchMemberIds(role: RoleResolvable): Promise<Snowflake[]> {
   ...
   *   .catch(console.error);
   */
  async setPosition(role: RoleResolvable, position: number, { relative, reason } = {} as SetRolePositionOptions): Promise<Role> {
    role = this.resolve(role);
    if (!role) throw new TypeError('INVALID_TYPE', 'role', 'RoleResolvable');
   ...
   *  .catch(console.error);
   */
  async setPositions(rolePositions: GuildRolePosition[]): Promise<Guild> {
