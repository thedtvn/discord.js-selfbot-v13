...
const validateName = (stringName: string): string =>
  s
    .string()
...
class TextBasedChannel {
  ...
  searchInteraction(): Promise<{ applications: any[]; application_commands: any[] }> {
  ...
  async sendSlash(botOrApplicationId: any, commandNameString: any, ...args: any[]): Promise<any> {
   ...
   * @returns {Promise<this>}
   */
  setNSFW(nsfw: boolean = true, reason?: string): any {
    return this.edit({ nsfw }, reason);
  }

  static applyToClass(structure: any, full: boolean = false, ignore: string[] = []): void {
    const props = ['send'];
    if (full) {
      props.push(
        'sendSlash',
...
// Utils
function parseChoices(parent: string, list_choices: any[], value: any): any {
  if (value !== undefined) {
...
async function addDataFromAttachment(value: any, client: any, channelId: string, attachments: any[]): Promise<{ id: number; attachments: any[] }> {
...
function awaitAutocomplete(client: any, nonce: string, defaultValue: any): Promise<any> {
