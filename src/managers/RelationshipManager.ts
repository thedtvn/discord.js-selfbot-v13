...
class RelationshipManager extends BaseManager {
  public cache: Collection<Snowflake, number>;
   ...
   */
  toJSON(): Array<{ id: Snowflake; type: string; nickname: string | undefined; since: string }> {
   ...
   */
  async fetch(
    user?: ThreadMember | GuildMember | Message | User | string,
   ...
   */
  async deleteRelationship(user: ThreadMember | GuildMember | Message | User | string): Promise<boolean> {
    const id = this.resolveId(user);
    if (
   ...
   * @returns {Promise<boolean>}
   */
  async deleteIgnored(user: ThreadMember | GuildMember | Message | User | string): Promise<boolean> {
   ...
   * @returns {Promise<boolean>}
   */
  async sendFriendRequest(options: ThreadMember | GuildMember | Message | User | string): Promise<boolean> {
   ...
   * @returns {Promise<boolean>}
   */
  async addFriend(user: ThreadMember | GuildMember | Message | User | string): Promise<boolean> {
    const id = this.resolveId(user);
   ...
   * @returns {Promise<boolean>}
   */
  async setNickname(user: ThreadMember | GuildMember | Message | User | string, nickname: string | null = null): Promise<boolean> {
    const id = this.resolveId(user);
    if (this.cache.get(id) !== RelationshipTypes.FRIEND) return Promise.resolve(false);
   ...
   * @returns {Promise<boolean>}
   */
  async addBlocked(user: ThreadMember | GuildMember | Message | User | string): Promise<boolean> {
    const id = this.resolveId(user);
    // Check
   ...
   * @returns {Promise<boolean>}
   */
  async addIgnored(user: ThreadMember | GuildMember | Message | User | string): Promise<boolean> {
    const id = this.resolveId(user);
    // Check
