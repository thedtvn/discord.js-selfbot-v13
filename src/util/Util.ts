...
const isObject = (d: unknown): d is object => typeof d === 'object' && d !== null;
...
class Util extends null {
   ...
   * @returns {string}
   */
  static escapeEscape(text: string) {
    return text.replaceAll('\\', '\\\\');
  }
   ...
   * @returns {string}
   */
  static escapeHeading(text: string) {
   ...
   * @returns {string}
   */
  static escapeBulletedList(text: string) {
   ...
   * @returns {string}
   */
  static escapeNumberedList(text: string) {
   ...
   * @private
   */
  static resolvePartialEmoji(emoji: { id?: string; name?: string; animated?: boolean } | string | null) {
   ...
   */
  static resolveColor(color: string | number | [number, number, number]) {
  ...
  static async setPosition(
    ...
    position: number,
    relative: boolean,
    sorted: Collection<string, { id: string }>,
  ...
  static _removeMentions(str: string) {
   ...
   * @ignore
   */
  static transformAPIGuildForumTag(tag: { id: string; name: string; moderated: boolean; emoji_id: string | null; emoji_name: string | null }) {
    return {
      id: tag.id,
      name: tag.name,
   ...
   * @ignore
   */
  static transformGuildForumTag(tag: { id: string; name: string; moderated: boolean; emoji?: { id?: string | null; name?: string | null } | null }) {
    return {
      id: tag.id,
      name: tag.name,
      moderated: tag.moderated,
   ...
   * @ignore
   */
  static transformAPIGuildDefaultReaction(defaultReaction: { emoji_id: string | null; emoji_name: string | null }) {
    return {
   ...
   * @ignore
   */
  static transformGuildDefaultReaction(defaultReaction: { id: string | null; name: string | null }) {
    return {
   ...
   * @returns {number}
   */
  static calculateUserDefaultAvatarIndex(userId: string) {
  ...
  static uploadFile(data: Buffer | NodeJS.ReadableStream | string, url: string) {
  ...
  static createPromiseInteraction(client: any, nonce: string, timeoutMs = 5_000, isHandlerDeferUpdate = false, parent?: unknown): Promise<unknown> {
    return new Promise((resolve, reject) => {
