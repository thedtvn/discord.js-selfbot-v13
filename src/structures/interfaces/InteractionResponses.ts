...
import { InteractionResponseTypes, InteractionTypes } from '../../util/Constants';
...
class InteractionResponses {
   ...
   *   .catch(console.error);
   */
  fetchReply(message: string = '@original'): any {
   ...
   *   .catch(console.error);
   */
  async deleteReply(message: string = '@original'): Promise<void> {
   ...
   *   .catch(console.error);
   */
  async deferUpdate(options: any = {}): Promise<any> {
    if (this.deferred || this.replied) throw new Error('INTERACTION_ALREADY_REPLIED');
    await this.client.api.interactions(this.id, this.token).callback.post({
      data: {
