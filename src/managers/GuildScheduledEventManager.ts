...
import { PrivacyLevels, GuildScheduledEventEntityTypes, GuildScheduledEventStatuses } from '../util/Constants';
...
> {
   ...
   */
  async create(options: GuildScheduledEventCreateOptions): Promise<GuildScheduledEvent> {
  ...
  ): Promise<Collection<Snowflake, GuildScheduledEventUser>> {
    const guildScheduledEventId = this.resolveId(guildScheduledEvent);
    if (!guildScheduledEventId) throw new Error('GUILD_SCHEDULED_EVENT_RESOLVE');
