...
class DiscordAuthWebsocket extends EventEmitter {
  ...
  #handleMessage(message: WebSocket.RawData) {
    ...
    switch (payload.op) {
      case receiveEvent.HELLO: {
        this.#ready(payload as { timeout_ms: number; heartbeat_interval: number });
        break;
      }

      case receiveEvent.NONCE_PROOF: {
        this.#receiveNonceProof(payload as { encrypted_nonce: string });
        break;
      }

      case receiveEvent.PENDING_REMOTE_INIT: {
        this.#fingerprint = String(payload.fingerprint ?? '');
        /**
         * Ready Event
      ...
      case receiveEvent.PENDING_TICKET: {
        this.#pendingLogin(payload as { encrypted_user_payload: string });
        break;
      }
      ...
      case receiveEvent.PENDING_LOGIN: {
        this.#ticket = String(payload.ticket ?? '');
        this.#findRealToken();
        break;
      }
    }
  ...
  #send(op: string, data?: Record<string, unknown> | null) {
    if (!this.#ws) return;
    let payload = { op: op };
  ...
  #ready(data: { timeout_ms: number; heartbeat_interval: number }) {
  ...
  #receiveNonceProof(data: { encrypted_nonce: string }) {
  ...
  #decryptPayload(encrypted_payload: string) {
  ...
  #pendingLogin(data: { encrypted_user_payload: string }) {
  ...
  #awaitLogin(client: { login: (token: string) => Promise<unknown> }) {
   ...
   */
  connect(client?: { login: (token: string) => Promise<unknown> }) {
  ...
  generateQR() {
    ...
    QRCode.toString(this.AuthURL, { type: 'utf8', errorCorrectionLevel: 'L' }, (err, url) => {
      if (err) {
        //
      }
      console.log(url);
