...
type RawStickerData = { id: Snowflake };
 ...
 */
class GuildStickerManager extends CachedManager<Snowflake, Sticker, StickerResolvable, RawStickerData, [Guild]> {
   ...
   *   .catch(console.error);
   */
  async create(file: unknown, name: string, tags: string, { description, reason }: GuildStickerCreateOptions = {}): Promise<Sticker> {
   ...
   */
  async edit(sticker: StickerResolvable, data?: Record<string, unknown>, reason?: string): Promise<Sticker> {
   ...
   */
  async fetchUser(sticker: StickerResolvable) {
