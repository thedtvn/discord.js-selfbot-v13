...
import { RangeError, Error as DjsError } from '../errors';
import ActivityFlags from '../util/ActivityFlags';
import { PollLayoutTypes, MessageReferenceTypes } from '../util/Constants';
...
class MessagePayload {
  ...
  public files: Record<string, any>[] | null;
   ...
   */
  async resolveFiles(): Promise<this> {
    if (this.files) return this;

    this.files = await Promise.all(this.options.files?.map((file: any) => (this.constructor as typeof MessagePayload).resolveFile(file)) ?? []);
    return this;
  }
   ...
   */
  static async resolveFile(fileLike: any): Promise<Record<string, any>> {
    let attachment;
    let name;
