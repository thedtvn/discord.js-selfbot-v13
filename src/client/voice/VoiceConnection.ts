...
import VoiceUDP from './networking/VoiceUDPClient';
import VoiceWebSocket from './networking/VoiceWebSocket';
import MediaPlayer from './player/MediaPlayer';
import VoiceReceiver from './receiver/Receiver';
import { parseStreamKey } from './util/Function';
import PlayInterface from './util/PlayInterface';
import Silence from './util/Silence';
import { Error } from '../../errors';
import { Opcodes, VoiceOpcodes, VoiceStatus, Events } from '../../util/Constants';
...
const SUPPORTED_MODES: string[] = ['aead_xchacha20_poly1305_rtpsize'];
...
const SUPPORTED_CODECS: string[] = ['VP8', 'H264'];
...
class VoiceConnection extends EventEmitter {
  ...
  connect(): void {
    ...
    ws.on('debug', (msg: string) => this.emit('debug', msg));
    udp.on('debug', (msg: string) => this.emit('debug', msg));
    ws.on('error', (err: Error) => this.emit('error', err));
    udp.on('error', (err: Error) => this.emit('error', err));
    ...
    /**
{
  ...
  playAudio(..._args: any[]): any {} // eslint-disable-line no-empty-function
  playVideo(..._args: any[]): any {} // eslint-disable-line no-empty-function
    ...
    return new Promise((resolve, reject) => {
      if (this.streamConnection) {
        ...
        if (!this.eventHook) {
          ...
          this.channel.client.on('raw', (packet: any) => {
        ...
        connection.once('failed', (reason: any) => {
          this.streamConnection = null;
          reject(reason);
        });
   ...
   */
  async joinStreamConnection(user: any): Promise<StreamConnectionReadonly> {
...
class StreamConnection extends VoiceConnection {
   ...
   * @returns {void}
   */
  sendScreenshareState(isPaused = false): void {
