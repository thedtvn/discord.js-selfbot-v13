...
class GuildManager extends CachedManager<Snowflake, Guild> {
   ...
   */
  async fetch(options: Record<string, unknown> = {}): Promise<Guild | Collection<Snowflake, OAuth2Guild>> {
  ...
  async setIncidentActions(
    guild: unknown,
    { invitesDisabledUntil, dmsDisabledUntil }: { invitesDisabledUntil?: string | number | Date | null; dmsDisabledUntil?: string | number | Date | null },
