...
class Options extends null {
  public static defaultSweeperSettings: Record<string, unknown>;

  /**
   * The default client options.
   * @returns {ClientOptions}
   */
  static createDefault(): Record<string, unknown> {
    return {
  ...
  static cacheWithLimits(settings: Record<string, number | Record<string, unknown>> = {}) {
    ...
    return (manager: { name: string }) => {
      ...
      if (typeof setting === 'number') {
        ...
        return new LimitedCollection({ maxSize: setting as number });
      }
      /* eslint-disable eqeqeq */
      const resolvedSetting = setting as {
        sweepFilter?: unknown;
        sweepInterval?: number;
        maxSize?: number;
      };
      const noSweeping =
        resolvedSetting.sweepFilter == null ||
        resolvedSetting.sweepInterval == null ||
        resolvedSetting.sweepInterval <= 0 ||
        resolvedSetting.sweepInterval === Infinity;
      const noLimit = resolvedSetting.maxSize == null || resolvedSetting.maxSize === Infinity;
   ...
   */
  static get defaultMakeCacheSettings(): Record<string, unknown> {
    return {
      MessageManager: 200,
      ChannelManager: {
        sweepInterval: 3600,
