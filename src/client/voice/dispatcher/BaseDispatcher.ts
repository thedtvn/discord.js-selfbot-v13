...
import secretbox from '../util/Secretbox';
...
class BaseDispatcher extends Writable {
  ...
  constructor(player: any, highWaterMark = 12, payloadType: number, extensionEnabled: boolean, streams: any = {}) {
    super({
      highWaterMark,
    });
  ...
  _playChunk(chunk: Buffer, isLastPacket = false): void {
    if (
