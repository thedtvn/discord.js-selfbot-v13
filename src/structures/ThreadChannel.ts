...
class ThreadChannel extends Channel {
  ...
  _patch(data: any, partial: boolean = false): any {
    ...
    if (Array.isArray(data.messages)) for (const message of data.messages) this.messages._add(message);
  }
   ...
   * @readonly
   */
  get guildMembers(): any /* Collection<Snowflake, GuildMember> */ {
   ...
   */
  async fetchOwner({ cache = true, force = false }: { cache?: boolean; force?: boolean } = {}): Promise<any> {
    if (!force) {
  ...
  async fetchStarterMessage(options?: any): Promise<any> {
   ...
   *   .catch(console.error);
   */
  setArchived(archived: boolean = true, reason?: string): Promise<ThreadChannel> {
   ...
   * @returns {Promise<ThreadChannel>}
   */
  async setInvitable(invitable: boolean = true, reason?: string): Promise<ThreadChannel> {
   ...
   *   .catch(console.error);
   */
  setLocked(locked: boolean = true, reason?: string): Promise<ThreadChannel> {
