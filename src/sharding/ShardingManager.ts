...
class ShardingManager extends EventEmitter {
  ...
  constructor(file: string, options: ShardingManagerOptions = {}) {
    ...
    if (!isAbsolute(file)) this.file = resolve(process.cwd(), file);
   ...
   */
  async spawn({ amount = this.totalShards, delay = 5500, timeout = 30_000 }: MultipleShardSpawnOptions = {}): Promise<Collection<number, Shard>> {
   ...
   */
  async broadcastEval(script: (client: unknown, context: unknown) => unknown, options: BroadcastEvalOptions = {}): Promise<unknown | unknown[]> {
   ...
   * @private
   */
  async _performOnShards(method: 'eval' | 'fetchClientValue', args: unknown[], shard?: number): Promise<unknown | unknown[]> {
   ...
   * @returns {Promise<Collection<number, Shard>>}
   */
  async respawnAll({ shardDelay = 5_000, respawnDelay = 500, timeout = 30_000 }: MultipleShardRespawnOptions = {}): Promise<Collection<number, Shard>> {
    let s = 0;
