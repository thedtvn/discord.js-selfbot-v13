...
import { H264NalSplitter, H265NalSplitter } from './processing/AnnexBNalSplitter';
import { IvfTransformer } from './processing/IvfSplitter';
import { H264Dispatcher } from '../dispatcher/AnnexBDispatcher';
import AudioDispatcher from '../dispatcher/AudioDispatcher';
import { VP8Dispatcher } from '../dispatcher/VPxDispatcher';

const FFMPEG_OUTPUT_PREFIX: string[] = ['-use_wallclock_as_timestamps', '1', '-copyts', '-analyzeduration', '0'];
const FFMPEG_INPUT_PREFIX: string[] = [
  '-reconnect',
  '1',
  '-reconnect_at_eof',
  '1',
...
const FFMPEG_PCM_ARGUMENTS: string[] = ['-f', 's16le', '-ar', '48000', '-ac', '2'];
const FFMPEG_VP8_ARGUMENTS: string[] = ['-f', 'ivf', '-deadline', 'realtime', '-c:v', 'libvpx'];
const FFMPEG_H264_ARGUMENTS = (options?: any): string[] => [
  '-c:v',
  'libx264',
  '-f',
  'h264',
...
const FFMPEG_H265_ARGUMENTS = (options?: any): string[] => [
  '-c:v',
  'libx265',
  '-f',
  'hevc',
...
class MediaPlayer extends EventEmitter {
  ...
  playUnknown(input: ReadableStream | string, options: any, streams: any = {}): AudioDispatcher {
    ...
    if (!isStream) args.unshift('-i', input as string);
    ...
    ffmpeg.process.stderr.on('data', (data: Buffer) => {
    ...
    if (isStream) {
      streams.input = input;
      (input as ReadableStream).pipe(ffmpeg);
    }
  ...
  playAnnexBVideo(stream: any, options: any, streams: any, type: string): any {
    this.destroyVideoDispatcher();
    let videoStream: H264NalSplitter | H265NalSplitter | undefined;
    if (type === 'H264') {
