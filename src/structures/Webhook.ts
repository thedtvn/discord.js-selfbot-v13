...
class Webhook {
   ...
   */
  async sendSlackMessage(body: any): Promise<boolean> {
    if (!this.token) throw new Error('WEBHOOK_TOKEN_UNAVAILABLE');
   ...
   */
  async edit({ name = this.name, avatar, channel }: { name?: string; avatar?: any; channel?: any }, reason?: string): Promise<this> {
   ...
   */
  async fetchMessage(message: string, cacheOrOptions: { cache?: boolean; threadId?: string } | boolean = { cache: true }): Promise<any> {
   ...
   * {@link WebhookClient} or if the channel is uncached, otherwise a {@link Message} will be returned
   */
  async editMessage(message: any, options: any): Promise<any> {
    if (!this.token) throw new Error('WEBHOOK_TOKEN_UNAVAILABLE');
   ...
   * @returns {Promise<void>}
   */
  async deleteMessage(message: any, threadId?: string): Promise<void> {
    if (!this.token) throw new Error('WEBHOOK_TOKEN_UNAVAILABLE');
   ...
   * @returns {?string}
   */
  avatarURL({ format, size }: { format?: string; size?: number } = {}): string | null {
