...
import DiscordAuthWebsocket from '../util/RemoteAuth';
...
export interface ClientOptions extends BaseClientOptions {
  ws: Record<string, unknown> & { presence?: unknown };
...
class Client extends BaseClient {
  declare public options: ClientOptions;

  public _cleanups: Set<() => void>;

  public _finalizers: FinalizationRegistry<CleanupPayload>;
  ...
  public user: ClientUserLike | null;
   ...
   */
  async passLogin(email: string, password: string): Promise<string | null> {
   ...
   *   .catch(console.error);
   */
  async fetchInvite(invite: string, options?: ClientFetchInviteOptions): Promise<Invite> {
   ...
   *   .catch(console.error);
   */
  async fetchGuildTemplate(template: string): Promise<GuildTemplate> {
   ...
   *   .catch(console.error);
   */
  async fetchWebhook(id: string, token?: string): Promise<Webhook> {
   ...
   *   .catch(console.error);
   */
  async fetchSticker(id: string): Promise<Sticker> {
   ...
   *   .catch(console.error);
   */
  async fetchPremiumStickerPacks(): Promise<Collection<string, StickerPack>> {
   ...
   */
  sweepMessages(lifetime: number = this.options.messageCacheLifetime as number): number {
   ...
   */
  async fetchGuildPreview(guild: unknown): Promise<GuildPreview> {
   ...
   */
  async fetchGuildWidget(guild: unknown): Promise<Widget> {
    const id = this.guilds.resolveId(guild);
    if (!id) throw new TypeError('INVALID_TYPE', 'guild', 'GuildResolvable');
    ...
    /**
{
  ...
  async acceptInvite(
    invite: string,
    options: AcceptInviteOptions = { bypassOnboarding: true, bypassVerify: true },
  ): Promise<unknown> {
   ...
   */
  deauthorize(id: string, type: 'application' | 'token' = 'application'): Promise<void> {
    if (type === 'application') {
   ...
   * @private
   */
  _validateOptions(options: ClientOptions = this.options as ClientOptions): void {
