...
class Shard extends EventEmitter {
  ...
  private _exitListener: ((...args: unknown[]) => void) | null;
   ...
   */
  spawn(timeout = 30_000): Promise<Child> {
    ...
    if (this.manager.mode === 'process') {
      this.process = fork(resolve(this.manager.file), this.args, {
          env: this.env,
      ...
      this.worker = new Worker(resolve(this.manager.file), { workerData: this.env })
        .on('message', this._handleMessage.bind(this))
        .on('exit', this._exitListener);
    }
   ...
   * @returns {Promise<ChildProcess>}
   */
  async respawn({ delay = 500, timeout = 30_000 }: ShardRespawnOptions = {}): Promise<Child> {
    this.kill();
  ...
  async fetchClientValue(prop: string): Promise<unknown> {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      this.send({ _fetchProp: prop }).catch((err: unknown) => {
        child.removeListener('message', listener);
        this.decrementMaxListeners(child);
        this._fetches.delete(prop);
        reject(err);
   ...
   */
  async eval(script: string | ((client: unknown, context: unknown) => unknown), context?: unknown): Promise<unknown> {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      this.send({ _eval }).catch((err: unknown) => {
        child.removeListener('message', listener);
        this.decrementMaxListeners(child);
        this._evals.delete(_eval);
        reject(err);
  ...
  _handleMessage(message: MessagePayload | null | undefined): void {
    if (message) {
      ...
      if (message._sFetchProp) {
        ...
        this.manager.fetchClientValues(message._sFetchProp as string, message._sFetchPropShard as number | undefined).then(
      ...
      if (message._sEval) {
        ...
        this.manager._performOnShards('eval', [message._sEval], message._sEvalShard as number | undefined).then(
          results => this.send({ ...resp, _result: results }),
          err => this.send({ ...resp, _error: Util.makePlainError(err) }),
        );
        return;
      ...
      if (message._sRespawnAll) {
        const { shardDelay, respawnDelay, timeout } = message._sRespawnAll as {
          shardDelay?: number;
          respawnDelay?: number;
          timeout?: number;
        };
   ...
   * @private
   */
  _handleExit(respawn = this.manager.respawn, timeout?: number): void {
    /**
    ...
    if (respawn) this.spawn(timeout).catch((err: unknown) => this.emit('error', err));
  }
