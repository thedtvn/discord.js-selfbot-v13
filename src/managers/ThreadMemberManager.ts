...
type UserResolvableLike = Snowflake | { id?: Snowflake };
type ThreadMemberResolvable = ThreadMember | UserResolvableLike;
type RawThreadMemberData = { user_id: Snowflake } & Record<string, unknown>;
...
class ThreadMemberManager extends CachedManager<Snowflake, ThreadMember, ThreadMemberResolvable, RawThreadMemberData> {
  ...
  _add(data: RawThreadMemberData, cache = true): ThreadMember {
   ...
   * @returns {Promise<Snowflake>}
   */
  async remove(id: Snowflake | '@me', reason?: string): Promise<Snowflake | '@me'> {
    await this.client.api.channels(this.thread.id, 'thread-members', id).delete({ reason });
    return id;
  }

  async _fetchOne(memberId: Snowflake, { cache, force = false, withMember }: FetchThreadMemberOptions): Promise<ThreadMember> {
    if (!force) {
  ...
  async _fetchMany({ cache, limit, after, withMember }: FetchThreadMemberOptions = {}): Promise<Collection<Snowflake, ThreadMember>> {
   ...
   */
  fetch(
    member?: ThreadMemberResolvable | boolean,
    options: FetchThreadMemberOptions = { cache: true, force: false },
